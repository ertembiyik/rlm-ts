Generally I just ask that you avoid touching `src/core/` files unless necessary. I'd like to keep the repo as minimal as possible for as long as possible so it's still easy for users to read the entire repo in a short sitting.
...
## Urgent TODOs
- [ ] **Additional Sandboxes**. Any more interesting, commonly used sandboxes (e.g. Apple Virtualization Framework, Vercel Sandboxes).
...
- [ ] **Add unit tests.** We need comprehensive unit tests using `bun test`.
- [ ] **Do more comprehensive bug finding**: Just find bugs and report them, we'll try to squash them all

## Would-be-nice TODOs
- [ ] **Multi-modal / arbitrary input support.** As it stands, we just support `string` / standard LM dict messages, but we should generally support any type of serializable inputs.
- [ ] **File-system based environments**. Beyond REPLs, we can also think about supporting filesystem + bash as a new type of sandbox. There seems to be a lot of interest in this.
